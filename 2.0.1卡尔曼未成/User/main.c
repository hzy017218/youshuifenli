...
int main(void)
{
  ...
  /*设置电机占空比--->设置电机速度为 30% */
  __IO uint16_t ChannelPulse = PWM_MAX_PERIOD_COUNT * 0.3;
  /* HAL库初始化*/
  HAL_Init();
  /* 初始化GPIO */
  HX711_GPIO_Init();
...
void HAL_SYSTICK_Callback(void)
{
  ...
  if (i == t) /* t ms计算一次 */
  {
    ...
    /* 电机牵引距离 */
    float DistancePerUnitTime = (Shaft_Speed / REDUCTION_RATIO) * D * pi * (t * 0.1); /* 单位时间牵引距离 = (输出轴转速 * 电机牵引轮半径 * π) * 单位时间 */
    ...
    printf("电机牵引距离：%.2f 米\r\n", TotalDistance);//电机牵引距离 = 单位时间牵引距离 * 单位时间 t（ms）
    printf("毛皮重量%d\r\n",Weight_Maopi);
    printf("拉力传感器数值: %d g\n", Weight_Shiwu);
    /* 记录当前总计数值，供下一时刻计算使用 */
